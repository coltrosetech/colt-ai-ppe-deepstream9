...
from validation import deepstream91_dgpu_runtime_smoke_r4 as smoke


@pytest.fixture(scope="session")
def plan() -> dict:
    return smoke.strict_json(smoke.ROOT / smoke.PLAN_REL, "R4 V6 plan")


@pytest.fixture(scope="session")
def outer() -> dict:
    return smoke.strict_json(smoke.ROOT / smoke.OUTER_REL, "R4 outer acceptance")


@pytest.fixture(scope="session")
def handoff() -> dict:
    return smoke.strict_json(smoke.ROOT / smoke.HANDOFF_REL, "R4 author handoff")


@pytest.fixture(scope="session")
def handoff_schema() -> dict:
    return smoke.strict_json(smoke.ROOT / smoke.HANDOFF_SCHEMA_REL, "R4 handoff schema")
...
def test_strict_json_rejects_duplicate_key(tmp_path: Path) -> None:
    ...
    with pytest.raises(smoke.SmokeR4AuthorError, match="duplicate JSON key"):
...
def test_strict_json_rejects_nonfinite(tmp_path: Path, token: bytes) -> None:
    ...
    with pytest.raises(smoke.SmokeR4AuthorError, match="non-finite JSON"):
...
def test_strict_json_rejects_nonobject(tmp_path: Path) -> None:
    ...
    with pytest.raises(smoke.SmokeR4AuthorError, match="JSON root differs"):
...
def test_cdi_void_environment_and_exact_gpu_request_are_bound(plan: dict) -> None:
    argv = plan["workload"]["argv"]
...
def test_only_worker_and_output_are_bind_mounted(plan: dict) -> None:
    ...
    assert sum(item.endswith("dst=/output") for item in mounts) == 1
    assert all(",rw" not in item for item in mounts)
...
def test_outer_acceptance_records_user_notification(outer: dict) -> None:
    assert outer["user_notification"] == {
        "confirmed": True,
        "notification_id": "1028ab1ffda984cd36be010dfde7abab67af803619cb6cf37527ee11c908e62d",
        "notified_at_utc": "2026-07-20T15:14:14Z",
    }
...
def test_r4_worker_pin_is_derived_and_exact(plan: dict) -> None:
...
def test_r4_worker_is_exact_minimal_derivation_from_r1() -> None:
    r1 = (smoke.ROOT / "validation/deepstream91_dgpu_runtime_smoke_probe_r1.py").read_text(
        encoding="utf-8"
    )
    expected = r1.replace("GPU Lease V5 plan", "GPU Lease V6 plan")
    expected = expected.replace(
        'visible == GPU_UUID, "NVIDIA_VISIBLE_DEVICES differs"',
        'visible == "void", "NVIDIA_VISIBLE_DEVICES differs"',
    )
    expected = expected.replace(
        '"NVIDIA_VISIBLE_DEVICES": GPU_UUID,',
        '"NVIDIA_VISIBLE_DEVICES": "void",',
    )
    observed = (smoke.ROOT / smoke.WORKER_REL).read_text(encoding="utf-8")
    assert observed == expected


def test_r4_worker_keeps_exact_real_gpu_identity_proof() -> None:
    source = (smoke.ROOT / smoke.WORKER_REL).read_text(encoding="utf-8")
    assert '"required_tokens": [GPU_UUID, DRIVER_VERSION]' in source
    assert 'visible == "void"' in source
    assert '"NVIDIA_VISIBLE_DEVICES": "void"' in source


def test_plan_binds_both_terminal_failure_receipts(plan: dict) -> None:
    artifacts = plan["workload"]["artifacts"]
    assert len(artifacts) == 4
    assert artifacts[2]["sha256"] == smoke.R2_FAILURE_RECEIPT_PIN["sha256"]
    assert artifacts[3]["sha256"] == smoke.R3_FAILURE_RECEIPT_PIN["sha256"]
    assert artifacts[2]["mode"] == artifacts[3]["mode"] == "0600"


@pytest.mark.parametrize(
    ("relative", "receipt_id", "receipt_fingerprint", "returncode"),
    [
        (
            smoke.R2_FAILURE_RECEIPT_REL,
            smoke.R2_FAILURE_RECEIPT_ID,
            smoke.R2_FAILURE_FINGERPRINT,
            125,
        ),
        (
            smoke.R3_FAILURE_RECEIPT_REL,
            smoke.R3_FAILURE_RECEIPT_ID,
            smoke.R3_FAILURE_FINGERPRINT,
            2,
        ),
    ],
)
def test_terminal_failure_lineage_is_exact(
    relative: str, receipt_id: str, receipt_fingerprint: str, returncode: int
) -> None:
    receipt = smoke.strict_json(smoke.ROOT / relative, "failure receipt")
    assert receipt["receipt_id"] == receipt_id
    assert receipt["receipt_fingerprint_sha256"] == receipt_fingerprint
    assert smoke.fingerprint(receipt, "receipt_fingerprint_sha256") == receipt_fingerprint
...
def test_r4_output_tree_is_fresh_and_absent() -> None:
...
def test_handoff_authority_is_nonterminal(handoff: dict) -> None:
    ...
    assert authority["independent_r4_review_accepted"] is False
...
def test_handoff_binds_failure_lineage_and_cdi_derivation(handoff: dict) -> None:
    assert handoff["failure_lineage"]["r2"]["returncode"] == 125
    assert handoff["failure_lineage"]["r3"]["returncode"] == 2
    assert handoff["failure_lineage"]["r2"]["preserved_immutable"] is True
    assert handoff["failure_lineage"]["r3"]["preserved_immutable"] is True
    worker = handoff["subject"]["worker"]
    assert worker["derived_from_r1_logic"] is True
    assert worker["nvidia_visible_devices_required"] == "void"
    assert worker["gpu_identity_proof"] == "nvidia-smi-exact-uuid-and-driver-output"
...
def test_handoff_requires_separate_review_and_live_gate(handoff: dict) -> None:
    assert handoff["required_next"] == [
        "separate_independent_review_of_exact_r4_author_bundle",
