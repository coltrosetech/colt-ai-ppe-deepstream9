...
from validation import gpu_lease_v5 as lease
...
def _materialize_plan(tmp_path: Path) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    plan: dict[str, Any] = {
        ...
        "v4_authority": {
            "contract": contract["base_v4"]["artifact"],
            "contract_fingerprint_sha256": lease.V4_CONTRACT_FINGERPRINT,
            "implementation": contract["base_v4"]["implementation"],
...
def test_contract_is_closed_and_frozen_v4_authority_is_exact() -> None:
    ...
    assert contract["base_v4"]["contract_fingerprint_sha256"] == lease.V4_CONTRACT_FINGERPRINT
    assert contract["base_v4"]["artifact"]["sha256"] == lease.V4_CONTRACT_SHA256
    assert contract["base_v4"]["implementation"]["sha256"] == lease.V4_SOURCE_SHA256
    ...
    assert lease.GPU_LEASE_V5_API_READY is False
...
def test_activation_receipt_binds_plan_effective_command_and_v4_terminal_pin(
    ...
    with _verify(path, expected) as verified:
        ...
        published = lease._write_activation_receipt(receipt, root=tmp_path / "v5-receipts")
        assert published["receipt_fingerprint_sha256"] == receipt[
            "receipt_fingerprint_sha256"
        ]
        final = tmp_path / "v5-receipts/gpu-0/receipts" / f"{receipt['receipt_id']}.json"
...
def test_repository_publishes_no_plan_or_v5_receipt() -> None:
    root = ROOT / "validation/results/gpu-leases/v5"
    assert root.is_dir()
