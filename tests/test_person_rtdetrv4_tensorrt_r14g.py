...
from validation import person_rtdetrv4_tensorrt_r14g as lane
...
def repo_tmp_path() -> Path:
    context = tempfile.TemporaryDirectory(prefix=".person-r14g-test-", dir=lane.ROOT)
    try:
...
def _memfd(payload: bytes) -> int:
    descriptor = os.memfd_create("person-r14g-test", os.MFD_CLOEXEC)
...
def test_frozen_r14d_r14e_r14f_inventory_is_exact_and_r14g_outputs_absent() -> None:
    ...
    assert [len(observed[key]) for key in ("r14d", "r14e", "r14f")] == [11, 11, 4]
...
def test_noncanonical_plan_fails_before_any_held_open(
    ...
    with pytest.raises(lane.TensorRTR14GError, match="canonical DEFAULT_PLAN"):
...
def test_exact_r14e_projection_drift_closes_all_plan_descriptors(
    ...
    with pytest.raises(lane.TensorRTR14GError, match="projection drifted"):
...
def test_execute_gate_precedes_plan_open_run_creation_and_popen(
    ...
    with pytest.raises(lane.TensorRTR14GError, match="execution gate is closed"):
        lane.execute_stage(
...
def test_r14g_source_exposes_no_external_process_or_accelerator_launcher() -> None:
