...
from validation import pose_mmpose_yoloxpose_tensorrt_ds91_r13j_v6 as lane
...
@pytest.fixture(scope="session")
def r13i_template() -> dict[str, Any]:
    path = lane.PINS["r13i_template"]["path"]
...
def verify_mutated(
    ...
    r13i_template: dict[str, Any],
    mutation: Callable[[dict[str, Any]], None],
) -> None:
    value = copy.deepcopy(templates[profile])
    mutation(value)
    with lane.Reader() as reader:
        with pytest.raises(lane.PoseR13JV6Error):
            lane.verify_profile(profile, value, r13i_template, reader)
...
def test_strict_json_rejects_ambiguous_envelopes(raw: bytes, match: str) -> None:
    with pytest.raises(lane.PoseR13JV6Error, match=match):
...
def test_reader_rejects_unsafe_paths(path: str) -> None:
    with pytest.raises(lane.PoseR13JV6Error, match="unsafe path"):
...
def test_reader_rejects_hardlink(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(lane, "ROOT", tmp_path)
    with lane.Reader() as reader:
        with pytest.raises(lane.PoseR13JV6Error, match="regular identity differs"):
...
def test_reader_rejects_mode_mismatch(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with lane.Reader() as reader:
        with pytest.raises(lane.PoseR13JV6Error, match="pin metadata differs"):
...
def test_frozen_r13i_acceptance_is_unmodified() -> None:
    expected = lane.PINS["r13i_independent_receipt"]
...
@pytest.mark.parametrize("profile", [640, 960])
def test_pose_outputs_keypoints_and_contract_are_exact(
    templates: dict[int, dict[str, Any]], profile: int
) -> None:
    pose = templates[profile]["pose_contract"]
    assert pose["input_name"] == "input"
    assert pose["outputs"]["dets"]["shape"] == ["B", 100, 5]
    assert pose["outputs"]["keypoints"]["shape"] == ["B", 100, 17, 3]
    assert pose["outputs"]["keypoints"]["keypoint_count"] == 17
    assert pose["same_index_association"] is True
    assert pose["second_nms"] is False
...
def test_mounts_are_one_read_only_input_and_one_fresh_output(
    ...
    assert len(mounts) == 2
    assert mounts[0].endswith(f"dst=/input/pose-{profile}.onnx,readonly")
    assert mounts[1].endswith("dst=/output")
    assert not mounts[1].endswith(",rw")
    assert "/validation/work/pose-r13j-v6/" in mounts[1]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (960, "optimization_profile.input.opt.0", 11),
...
def test_forbidden_argv_mutations_fail_closed(
    ...
    verify_mutated(640, templates, r13i_template, mutate)


def test_writable_onnx_mount_mutation_fails_closed(
    templates: dict[int, dict[str, Any]], r13i_template: dict[str, Any]
) -> None:
    def mutate(value: dict[str, Any]) -> None:
        argv = value["future_v6_workload"]["argv"]
...
def test_docker29_output_mount_rw_field_mutation_fails_closed(
    templates: dict[int, dict[str, Any]], r13i_template: dict[str, Any]
) -> None:
    def mutate(value: dict[str, Any]) -> None:
        argv = value["future_v6_workload"]["argv"]
